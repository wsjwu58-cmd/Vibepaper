...
> **2026-08-26 补齐进度（后台与创意广场）**  
> - 广场：description/tags/viewCount、详情轮播、访客浏览、审核驳回必填原因  
> - 后台：`AdminLayout` 多路由；审核预览/批量；套餐/公告/模型/API Key/用户/审计/素材 block-unblock  
> - 冒烟：远程 Nacos `192.168.141.129:8848` 下 PaperHub 发布 → 驳回/通过 → 访客浏览 → 克隆 已通  
...
| 功能 | 状态 | 缺口说明 | PRD |
|------|------|----------|-----|
| 创意广场 · 成品预览 / 制作过程图 / 全屏 | **REAL（P1 主路径）** | 列表/详情/标签描述/浏览量/访客浏览、多结果轮播、React Flow 流程回放、PaperHub 发布与克隆已通；评论/点赞等未做 | P1 |
...
| 功能 | 状态 | 说明 |
|------|------|------|
| 用户管理 / 内容审核 / 模型 / 套餐 / 交易列表 / 公告 / 审计 / API Key / 素材治理 | **REAL（P1 主路径）** | `AdminLayout` 多路由侧栏；审核预览/驳回原因/批量；套餐公告模型 API Key 用户审计 CRUD/筛选已接；素材 block/unblock；不做独立 admin-web / 细粒度 RBAC / 外链 / 活动 |
...
- 任务历史筛选、分享链接、发布创意广场（描述/标签）、访客浏览与浏览量、详情轮播、流程回放、克隆  
- 运营后台多页（审核/用户/套餐/公告/模型/API Key/审计/交易/素材屏蔽） 
- 奖励签到/每日任务、邀请中心、公告  
...
| **P1** | 图/视频加工与 Seedance、合成/导演台接真任务、编组堆叠完整 UX、Skill 三创建法与会话史、创意广场与运营后台主路径已对齐参考（仍缺 RBAC/活动/外链等）、企业素材转入 UI |
| **P2** | 短长期记忆产品化、会员体系后台 |
