...
if (-not $SkipPython) {
    ...
    # Celery worker+beat：clock 唤醒 / 记忆队列（agent 进程内也有 inline scanner 兜底）
    $celery = "$root\agent-service\.venv\Scripts\celery.exe"
    if (Test-Path $celery) {
        Start-Process -FilePath $celery `
            -ArgumentList "-A", "src.agent.workers.celery_app.celery_app", "worker", "--beat", "-l", "info", "-P", "solo" `
            -WorkingDirectory "$root\agent-service" -WindowStyle Hidden `
